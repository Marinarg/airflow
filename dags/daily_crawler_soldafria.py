"""
## DAG Documentation
This DAG is responsible for the data ingestion of soldafria crawler
"""
...
move_files = BashOperator(
    ...
    bash_command=json_helper["bash_to_move_files"].format("soldafria.csv"),
    dag=dag
)
...
insert_csv_on_table_mysql = MySqlOperator(
    ...
    sql=json_helper["sql_to_save_csv"].format("soldafria.csv"),
    dag=dag
)

delete_csv_file = BashOperator(
    task_id="delete_csv_file",
    bash_command=json_helper["bash_to_delete_csv"].format("soldafria.csv"),
    dag=dag
)
