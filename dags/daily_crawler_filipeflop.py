"""
## DAG Documentation
This DAG is responsible for the data ingestion of filipeflop crawler
"""
...
trigger_crawler = BashOperator(
    ...
    bash_command=json_helper["bash_to_trigger_crawler"].format("filipeflop","filipeflop.csv"),
    dag=dag
)

move_files = BashOperator(
    task_id="move_result_files",
    bash_command=json_helper["bash_to_move_files"].format("filipeflop.csv"),
    dag=dag
)
...
insert_csv_on_table_mysql = MySqlOperator(
    ...
    sql=json_helper["sql_to_save_csv"].format("filipeflop.csv"),
    dag=dag
)
delete_csv_file = BashOperator(
    task_id="delete_csv_file",
    bash_command=json_helper["bash_to_delete_csv"].format("filipeflop.csv"),
    dag=dag
)
